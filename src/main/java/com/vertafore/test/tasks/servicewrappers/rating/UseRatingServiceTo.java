...
public class UseRatingServiceTo {
	...
	public static Performable getRateResponsesByCorrelationIdUsingGetOnTheRatingController(String correlationId, String pageSize, String page, String filter){
		...
		"{0} Retrieve Rate Responses by Correlation ID", 
			actor -> {
				CallTitanApi.asActorUsingService(actor, THIS_SERVICE).contentType("application/json").queryParam("correlationId", correlationId).queryParam("pageSize", pageSize).queryParam("page", page).queryParam("filter", filter).get("rates?filter=byCorrelationId");
			}
		);
	}
	...
	public static Performable getRateResponseByCorrelationIdAndRatingUnitIdUsingGetOnTheRatingController(String correlationId, String ratingUnitId, String filter){
		...
		"{0} Retrieve Rate Responses by Correlation ID and Rating Unit ID", 
			actor -> {
				CallTitanApi.asActorUsingService(actor, THIS_SERVICE).contentType("application/json").queryParam("correlationId", correlationId).queryParam("ratingUnitId", ratingUnitId).queryParam("filter", filter).get("rates?filter=byCorrelationIdAndRatingUnitId");
			}
		);
	}
	...
	public static Performable requestRateUsingPostOnTheRatingController(Object body){
		...
		"{0} Request Rate", 
			actor -> {
				CallTitanApi.asActorUsingService(actor, THIS_SERVICE).contentType("application/json").body(body).post("rates");
			}
		);
	}
